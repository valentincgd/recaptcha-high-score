...
<body onload="load('',false)">
  ...
  // ─────────────── MODE SERVICE CHAUD (RC_WARM_PORT) ───────────────
  // Boot une seule fois (fenêtre jsdom + script parsés), puis sert des tokens à la demande :
  // chaque requête = 1 execute() dans la MÊME fenêtre → on amortit le bootstrap (le plus cher).
  if (process.env.RC_WARM_PORT) {
    // un cycle execute() → token frais, en réutilisant la fenêtre chaude
    async function executeOnce(action) {
      if (action) ACTION = action;
      captures.token = null; captures.field16 = null; captures.reloadResponse = null; captures.reloadSummary = null;
      const g = grec();
      if (MOUSE) { try { await simulateMouse(window, Math.min(MOUSE_MS, 900)); } catch (_) {} }
      await new Promise((res) => {
        try { g.ready(() => { Promise.resolve(g.execute(SITE_KEY, { action: ACTION })).then(t => { captures.token = t; res(); }).catch(e => { log('execute-err', e.message); res(); }); }); }
        catch (e) { log('execute-err', e.message); res(); }
      });
      const dl = Date.now() + 9000;
      while (Date.now() < dl && !(captures.field16 && captures.reloadResponse)) await new Promise(r => setTimeout(r, 40));
      return { token: captures.token, field16: captures.field16, reloadStatus: captures.reloadResponse ? captures.reloadResponse.status : null };
    }
    // attendre grecaptcha prêt
    const dl0 = Date.now() + TIMEOUT;
    while (Date.now() < dl0 && !grecaptchaReady()) await new Promise(r => setTimeout(r, 100));
    if (!grecaptchaReady()) throw new Error('grecaptcha non prêt (warm)');
    if (PRE_EXECUTE_MS > 0) { try { await sessionWarmup(window, Math.min(PRE_EXECUTE_MS, 2500)); } catch (_) {} }
    // clientHints cohérents avec l'empreinte réellement chargée (IDENTITY = RC_CHROME_VERSION/RC_PLATFORM du profil)
    const clientHints = { user_agent: IDENTITY.userAgent, accept_lang: `${HL}-${HL.toUpperCase()},${HL};q=0.9,en-US;q=0.8,en;q=0.7`, sec_ch_ua: IDENTITY.secChUa, sec_ch_ua_mobile: IDENTITY.secChUaMobile, sec_ch_ua_platform: IDENTITY.secChUaPlatform };
    const http = require('http');
    let busy = false, served = 0;
    const server = http.createServer(async (req, res) => {
      if (req.url.startsWith('/health')) { res.writeHead(200); return res.end(JSON.stringify({ status: 'ok', served, sitekey: SITE_KEY, mode: MODE })); }
      if (busy) { res.writeHead(503, { 'content-type': 'application/json' }); return res.end('{"error":"busy"}'); }
      busy = true;
      try {
    ...
    await new Promise(r => server.listen(Number(process.env.RC_WARM_PORT), '127.0.0.1', r));
    log('warm', `service chaud prêt 127.0.0.1:${process.env.RC_WARM_PORT} (sitekey ${SITE_KEY.slice(0, 12)}…, mode ${MODE})`);
    process.stdout.write(`__WARM_READY__${process.env.RC_WARM_PORT}__\n`);
    await new Promise(() => {}); // le service vit indéfiniment (fenêtre chaude conservée)
    return;
  }
