...
(async()=>{
  ...
  const onP=async(pr)=>{const f=pr.callFrames,t=f[0]; if(!t||t.location.lineNumber!==652){await send('Debugger.resume').catch(()=>{});return;} if(caps.length>=3||busy){if(caps.length>=3&&bp){await send('Debugger.removeBreakpoint',{breakpointId:bp}).catch(()=>{});bp=null}await send('Debugger.resume').catch(()=>{});return;} busy=true;
    try{
      ...
      if(dv&&dv.length>5){
        const stack=f.slice(0,14).map(x=>(x.functionName||'?')+'@'+(x.location.lineNumber+1)+':'+x.location.columnNumber);
        caps.push({d:dv.slice(0,300), stack});
      }
  ...
  child.on('exit',()=>{if(fin)return;fin=true;fs.writeFileSync(path.join(__dirname,'..','scripts','t55.json'),JSON.stringify(caps,null,1));console.log('caps55:',caps.length);process.exit(0)});
  setTimeout(()=>{if(!fin){fin=true;fs.writeFileSync(path.join(__dirname,'..','scripts','t55.json'),JSON.stringify(caps,null,1));console.log('timeout caps55:',caps.length);try{child.kill()}catch(_){}process.exit(0)}},70000);
