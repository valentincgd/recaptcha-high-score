...
export class RecaptchaEnterprise {
  ...
  async #getTokenViaDynamic({ cfg, started }) {
    ...
    const profile = bindFingerprintSession(cfg);
    ...
    try {
      ...
      try {
        ...
        const flatBody = DynamicFlatReloadBuilder.build({
          version: bootstrap.version,
          anchorToken: anchorForReload.anchorToken,
          siteKey: cfg.siteKey,
          action: cfg.action,
          encryptionKey: anchorForReload.encryptionKey,
          configBytecode: anchorForReload.configBytecode,
          vmBytecodeKeys: anchorForReload.config?.vmBytecodeKeys,
        ...
        built = {
          body: flatBody,
          reloadBytes: flatBody.length,
          strategy: "dynamic-flat-retry",
          secondarySource: "flat-retry",
        };
        reload = await this.#postReload({
          headers,
          jar,
