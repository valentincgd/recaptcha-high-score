(function () {
  ...
  const observerWriter = {
    randNumber: 0,
    hasher1: new BinaryGridHasher(),
    hasher2: new BinaryGridHasher(),
  };

  const observer = new MutationObserver((mutationsList) => {
    mutationsList.forEach((mutation) => {
      if (Math.random() < 0.5) {
        observerWriter.randNumber++;
      }

      if (mutation.attributeName) {
        observerWriter.hasher1.add(mutation.attributeName);
      }

      if (mutation.target && mutation.target.tagName) {
        observerWriter.hasher2.add(mutation.target.tagName);
      }

      const finalValue = [
        observerWriter.randNumber,
        observerWriter.hasher1.toString(),
        observerWriter.hasher2.toString(),
      ];
